---
description: "Use when editing backend APIs, SSE streaming, route handlers, analysis normalization, Alibaba STS/OSS integration, or Node.js server logic under backend/. Preserve frontend-facing contracts and endpoint behavior."
...
- Treat SSE shape from `POST /api/analyze` as a compatibility contract for the Next.js frontend parser in [frontend/lib/backend.ts](../../frontend/lib/backend.ts).
- If you change output normalization (`organ_id`, `severity`, aliases), update both backend runtime logic and frontend parsing paths together.
